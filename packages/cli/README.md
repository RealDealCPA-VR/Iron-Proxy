...
`run <provider>` starts the vendor CLI as the account a request would use first right now, printing `Using "<title>" (<provider>)` on stderr, with the home variable set and every `*_API_KEY` removed, and exits with the CLI's exit code. `--profile id` picks the account. `env <provider>` prints `export` (bash/zsh, the default off Windows), `$env:` (PowerShell, the default on Windows) or `set "..."` (cmd) lines for the home variable, and for bash and PowerShell lines that clear the API-key variables. An interactive session cannot switch accounts mid-session: when it hits a limit, quit and `run` again to get the next ready account.
